document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        reader.onload = function(event) {
            ...
            const dataHora = new Date().toLocaleString(); // Registro de data e hora
    ...
    function adicionarItemNaLista(item, quantidade, precoLojas, categoria, imagemSrc, dataHora) {
        ...
        li.innerHTML = `
            <div>
                ...
                <small>${dataHora}</small> <!-- Exibe a data e hora -->
            </div>
    ...
    function salvarLista() {
        ...
        listaCompras.querySelectorAll('li').forEach(li => {
            ...
            const dataHora = li.querySelector('small').textContent; // Captura a data e hora

            itens.push({ nome: itemTexto, quantidade, precoLojas, categoria, imagem, dataHora });
        });

        localStorage.setItem('listaCompras', JSON.stringify(itens)); // Salva a lista no localStorage
    }

    // Função para exportar a lista em PDF
    botaoSalvar.addEventListener('click', function() {
        const doc = new jsPDF(); // Usando jsPDF para gerar PDF
        const listaTexto = [];
        ...
        listaCompras.querySelectorAll('li').forEach(li => {
            ...
            const categoria = li.querySelector('.categoria').textContent.replace(/[()]/g, '');
            const dataHora = li.querySelector('small').textContent;
            listaTexto.push(`${itemTexto} - Quantidade: ${quantidade} - Categoria: ${categoria} - Adicionado em: ${dataHora}`);
        });

        doc.text("Lista de Compras", 10, 10);
        listaTexto.forEach((texto, index) => {
